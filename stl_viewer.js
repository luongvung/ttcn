...
function StlViewer(parent_element_obj, options) {
  ...
  this.canvas_width = "100vh";
  this.canvas_height = "90vh";
  ...
  this.load_three_files = _this.get_opt(
    "load_three_files",
  ...
  this.set_drag_and_drop = function (b) {
    if (b) {
      ...
      _this.parent_element.removeEventListener(
        "dragover",
  ...
  this.set_options = function () {
    ...
    _this.model_loaded_callback = _this.get_opt(
      "model_loaded_callback",
      _this.model_loaded_callback
    );
    _this.all_loaded_callback = _this.get_opt(
      "all_loaded_callback",
      _this.all_loaded_callback
    );
    _this.load_error_callback = _this.get_opt(
      "load_error_callback",
      _this.load_error_callback
    );
    _this.loading_progress_callback = _this.get_opt(
      "loading_progress_callback",
      _this.loading_progress_callback
    );
    _this.onmousedown_callback = _this.get_opt(
      "on_model_mousedown",
      _this.onmousedown_callback
    );
    if (!_this.onmousedown_callback)
      _this.onmousedown_callback = _this.get_opt("on_model_mouseclick", null);
    _this.send_no_model_click_event = _this.get_opt(
      "send_no_model_click_event",
      _this.send_no_model_click_event
    );
    ...
    if (_this.zoom >= 0) _this.cameraz = _this.zoom;
    else _this.cameraz = _this.get_opt("cameraz", _this.cameraz);

    _this.camera_state = _this.get_opt("camera_state", _this.camera_state);

    //drag and drop?
    if (_this.allow_drag_and_drop) _this.set_drag_and_drop(true);
  ...
  this.load_model = function (model) {
    ...
    if (model.filename || model.local_file)
      return _this.load_from_stl_file(model, false);
  ...
  this.add_from_existing_mesh = function (model) {
    ...
    if (_this.model_loaded_callback) _this.model_loaded_callback(model.id);
  };

  this.load_from_stl_file = function (model) {
    var model_worker = new Worker(
      (typeof _this.load_three_files == "string"
        ? _this.load_three_files
        : "") + "load_stl.min.js"
    );
    ...
    model_worker.onmessage = function (e) {
      ...
      switch (e.data.msg_type) {
        case _this.MSGFROMWORKER_STL_LOADED:
          ...
          var geo = _this.vf_to_geo(
            e.data.vertices,
            e.data.faces,
            e.data.colors ? e.data.colors : false,
            model.color
          );
          if (geo) {
            ...
            var material = new THREE.MeshLambertMaterial({
              color: 0x909090,
              wireframe: false,
              vertexColors: model.color ? THREE.NoColors : THREE.FaceColors,
          ...
          if (_this.pre_loaded_ab_files)
            if (model.filename)
              if (_this.pre_loaded_ab_files[model.filename])
                delete _this.pre_loaded_ab_files[model.filename];
          break;
    ...
    if (_this.pre_loaded_ab_files)
      if (model.filename)
        if (_this.pre_loaded_ab_files[model.filename])
          blob_to_load = _this.pre_loaded_ab_files[model.filename];
    ...
    model_worker.postMessage({
      msg_type: _this.MSG2WORKER_DATA,
      data: model,
      load_from_blob_or_ab: blob_to_load,
      get_progress: _this.loading_progress_callback != null,
      jszip_path: _this.jszip_path,
    });
  ...
  this.model_loaded = function (model_id) {
    ...
    if (Object.keys(_this.loaded_models_arr).length >= _this.models_count) {
      ...
      if (!_this.camera_state) _this.set_zoom(); //camera state overrides zoom
      else _this.camera_state = null; //it is one time thingy (next bunch of models will have to set camera state again)
  ...
  this.set_grid = function (b, size, divisions) {
    ...
    if (b) {
      if (!size)
        size = Math.max(Math.abs(_this.maxx), Math.abs(_this.minx)) * 2.5;
      if (size <= 0) {
  ...
  this.remove_model = function (model_id) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("remove_model - id not found: " + model_id);
    ...
    Object.keys(_this.models_ref).forEach(function (key) {
      _this.max_model_id = Math.max(
        _this.models[_this.models_ref[key]].id,
        _this.max_model_id
      );
    });
  ...
  this.set_zoom = function (zoom, force_zoom) {
    ...
    if (_this.zoom_done && !force_zoom)
      //don't do zoom for more than once
      return;
  ...
  this.get_camera_state = function () {
    ...
    if (_this.controls) vtarget.copy(_this.controls.target);
  ...
  this.set_camera_state = function (state) {
    ...
    if (state.position !== undefined) {
      if (state.position.x === undefined)
        return _this.model_error("set_camera_state - invalid position x");
      if (state.position.y === undefined)
        return _this.model_error("set_camera_state - invalid position y");
      if (state.position.z === undefined)
        return _this.model_error("set_camera_state - invalid position z");
      _this.camera.position.set(
        state.position.x,
        state.position.y,
        state.position.z
    ...
    if (state.up !== undefined) {
      if (state.up.x === undefined)
        return _this.model_error("set_camera_state invalid up x");
      if (state.up.y === undefined)
        return _this.model_error("set_camera_state invalid up y");
      if (state.up.z === undefined)
        return _this.model_error("set_camera_state invalid up z");
    ...
    if (state.target !== undefined) {
      if (state.target.x === undefined)
        return _this.model_error("set_camera_state - invalid target x");
      if (state.target.y === undefined)
        return _this.model_error("set_camera_state - invalid target y");
      if (state.target.z === undefined)
        return _this.model_error("set_camera_state - invalid target z");
  ...
  this.check_loading_status = function (model, loaded, total) {
    if (model)
      _this.load_status[model.id] = {
        loaded: loaded,
        total: total,
        load_session: _this.load_session,
      };
  ...
  this.set_edges = function (model_id, b) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("set_edges - id not found: " + model_id);
  ...
  this.set_or_update_geo_edges = function (model, b, force_geo_recalc) {
    if (!b || force_geo_recalc) {
      if (model.edges) _this.scene.remove(model.edges);
    ...
    if (!model.edges || force_geo_recalc) {
      ...
      model.edges = new THREE.LineSegments(
        new THREE.EdgesGeometry(geo),
        _this.edges_material
      );
      add_to_scene = true;
    }
  ...
  this.set_model_custom_props = function (model) {
    ...
    //view edges?
    if (model.view_edges) _this.set_or_update_geo_edges(model, true);
    ...
    //animation
    if (model.animation) _this.animation[model.id] = 1;
  };
  ...
  this.set_scale = function (
    ...
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("set_scale - id not found: " + model_id);
    ...
    if (model.edges) _this.set_or_update_geo_edges(model, true, true);

    //console.log(model.scalex+"/"+model.scaley+"/"+model.scalez);

    if (keep_prev_scale) {
      //keep the previous scale (just change the geometry)
      model.scalex = prev_scalex;
      model.scaley = prev_scaley;
      model.scalez = prev_scalez;
    }
  ...
  this.recalc_dims = function (model) {
    ...
    _this.maxx = _this.maxx
      ? Math.max(_this.maxx, geo.maxx + model.x)
      : geo.maxx + model.x;
    _this.maxy = _this.maxy
      ? Math.max(_this.maxy, geo.maxy + model.y)
      : geo.maxy + model.y;
    _this.maxz = _this.maxz
      ? Math.max(_this.maxz, geo.maxz + model.z)
      : geo.maxz + model.z;
    _this.minx = _this.maxx
      ? Math.min(_this.minx, geo.minx + model.x)
      : geo.minx + model.x;
    _this.miny = _this.maxy
      ? Math.min(_this.miny, geo.miny + model.y)
      : geo.miny + model.y;
    _this.minz = _this.maxz
      ? Math.min(_this.minz, geo.minz + model.z)
      : geo.minz + model.z;
  };
  ...
  this.update_mesh_color = function (mesh, color, model_colors) {
    ...
    mesh.material.vertexColors = model_colors
      ? THREE.FaceColors
      : THREE.NoColors; //use model original face colors (from STL file)?  or user defined color (aka NoColors to specific faces)
  ...
  this.set_color = function (model_id, color) {
    ...
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("set_color - id not found: " + model_id);
  ...
  this.error_in_model = function (model) {
    if (!model.id && model.id != 0 && model.id != -1)
      return _this.model_error("missing id");
    ...
    if (!model.filename && !model.mesh && !model.local_file) {
      ...
      else return _this.model_error("missing filename or mesh");
    }
    if (_this.models_ref[model.id])
      return _this.model_error("such model ID already exists: " + model.id);
    return null;
  };
  ...
  this.set_display = function (model_id, display) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("set_display - id not found: " + model_id);

    var model = _this.models[_this.models_ref[model_id]];
    if (!model) return;

    _this.set_material_display(
      display,
      model.mesh.material,
      model.mesh.geometry
    );
  ...
  this.onmousedown = function (event) {
    ...
    switch (event.type) {
      case "touchstart":
        ...
        _this.mouse.x =
          ((touch.pageX - _this.parent_element.offsetLeft) /
            _this.parent_element.clientWidth) *
            2 -
          1;
        _this.mouse.y =
          -(
            (touch.pageY - _this.parent_element.offsetTop) /
            _this.parent_element.clientHeight
          ) *
            2 +
          1;
        break;
      ...
      default: //click
        _this.mouse.x =
          ((event.clientX - _this.parent_element.offsetLeft) /
            _this.parent_element.clientWidth) *
            2 -
          1;
        _this.mouse.y =
          -(
            (event.clientY - _this.parent_element.offsetTop) /
            _this.parent_element.clientHeight
          ) *
            2 +
          1;
  ...
  this.set_model_units = function (model_id, units, scale) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("set_model_units - id not found: " + model_id);
  ...
  this.set_position = function (model_id, x, y, z) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("set_position - id not found: " + model_id);
  ...
  this.set_rotation = function (
    ...
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("rotate - id not found: " + model_id);
    ...
    //if model has edges - we need to update it
    if (model.edges) _this.set_or_update_geo_edges(model, true);
  };
  ...
  this.add_model = function (new_model, dont_add_to_model_count) {
    ...
    if (model_filename) {
      switch (model_filename.split(".").pop()) {
        case "vsj":
          return _this.load_vsj(
            new_model.local_file ? new_model.local_file : model_filename
          );

        case "vsb":
          return _this.load_vsb(
            new_model.local_file ? new_model.local_file : model_filename
          );
    ...
    if (new_model.id == -1) new_model.id = ++_this.max_model_id;
  ...
  this.add_models = function (new_models) {
    ...
    model_keys.forEach(function (key) {
      var model_filename = _this.get_model_filename(
        new_models[model_keys[key]]
      );
      ...
      if (model_filename) {
        switch (model_filename.split(".").pop()) {
          ...
          default:
            if (_this.models_ref[new_models[key].id] === undefined)
              _this.models_count++;
        }
      } else if (_this.models_ref[new_models[key].id] === undefined) _this.models_count++;
    });
  ...
  this.calc_volume_and_area = function (geo, factor) {
    ...
    for (i = 0; i < len; i++) {
      ...
      a =
        geo.vertices[geo.faces[i].a].distanceTo(geo.vertices[geo.faces[i].b]) *
        factor;
      b =
        geo.vertices[geo.faces[i].b].distanceTo(geo.vertices[geo.faces[i].c]) *
        factor;
      c =
        geo.vertices[geo.faces[i].c].distanceTo(geo.vertices[geo.faces[i].a]) *
        factor;
      s = (a + b + c) / 2;
  ...
  this.get_model_info = function (model_id) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("get_model_info - id not found: " + model_id);
    ...
    var vol_and_area = model.mesh.geometry
      ? _this.calc_volume_and_area(
          model.mesh.geometry,
  ...
  this.get_vsj = function (as_js_obj, force_basename, for_vsb) {
    ...
    var data = {
      ...
      bg_color: _this.bg_color,
      camera_state: _this.get_camera_state(),
      auto_rotate: _this.auto_rotate,
      mouse_zoom: _this.mouse_zoom,
      auto_resize: _this.auto_resize,
      center_models: _this.center_models,
    };
    ...
    Object.keys(_this.models_ref).forEach(function (key) {
      ...
      if (for_vsb) {
        ...
        var curr_filename = _this.get_model_filename(
          model,
          true,
          force_basename
        );
  ...
  this.load_vsj = function (filename) {
    if (!filename) {
      ...
      return _this.model_error(
        "load_vsj - invalid filename" + filename,
        _this.load_error_callback
      );
    }
  ...
  this.get_stl_bin = function (model_id) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("get_stl_bin - id not found: " + model_id);
    ...
    var s = _this.padend(
      "Binary" + (model.colors ? " colored" : "") + " STL by viewstl.com",
      80,
      " "
    );
    for (var i = 0; i < 80; i++) {
    ...
    Object.keys(geo.faces).forEach(function (face_key) {
      ...
      if (model.colors)
        d.setUint16(
          pos,
          Math.ceil(fdata.color.r * 31) |
            (Math.ceil(fdata.color.g * 31) << 5) |
            (Math.ceil(fdata.color.b * 31) << 10),
          true
        );
  ...
  this.get_vsb_blob = function () {
    ...
    Object.keys(vsb.files).forEach(function (key) {
      ...
      var curr_filename = _this.get_model_filename(
        vsb.vsj.models[_this.models_ref[vsb.files[key].id]],
        true,
        true
      );
      ...
      zip.forEach(function (relativePath, zipEntry) {
        if (zipEntry.name == "json_data.vsj") {
          zip.files[zipEntry.name].async("string").then(function (fileData) {
            ...
            _this.zip_load_count--;
            if (_this.zip_load_count == 0) _this.load_vsj(null);
          });
        } else {
  ...
  this.download_model = function (model_id, filename) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("download_model - id not found: " + model_id);

    var model = _this.models[_this.models_ref[model_id]];
    if (!model) return;
    if (!model.mesh) return;

    var blob = new Blob([_this.get_stl_bin(model_id)], {
  ...
  this.get_model_mesh = function (model_id) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("get_model_mesh - id not found: " + model_id);
  ...
  this.animate = function () {
    ...
    if (_this.controls) _this.controls.update();
  };
  ...
  this.do_model_animation = function (model) {
    ...
    if (model.animation.delta) {
      var p =
        (curr_time - model.animation.start_time) / model.animation.delta.msec; //percentage
      var p_from_last_time = model.animation.last_time
        ? (curr_time - model.animation.last_time) / model.animation.delta.msec
        : p;
      _this.animation_next_delta(
        model,
        model.animation.delta,
        p_from_last_time
      );
      ...
      if (p >= 1) {
        ...
        //loop - start over
        else model.animation.delta.start_time = null;
      }
    }

    if (model.animation.exact) {
      var p_from_last_time =
        (curr_time -
          (model.animation.last_time
            ? model.animation.last_time
            : model.animation.start_time)) /
        model.animation.exact.msec;
      _this.animation_next_exact(
        model,
        model.animation.exact,
        p_from_last_time
      );

      if (
        curr_time >=
        model.animation.start_time + model.animation.exact.msec
      ) {
  ...
  this.animation_next_delta = function (model, a_data, inc_const) {
    ...
    Object.keys(a_data).forEach(function (key) {
      switch (key) {
        ...
        case "z":
          if (!done_position) {
            //we'll do all x/y/z at once (and one time ...)
            done_position = true;
            _this.set_position(
              model.id,
        ...
        case "scalez":
          if (!done_scale) {
            //we'll do all x/y/z at once (and one time ...)
            done_scale = true;
            a_data.scalex = a_data.scalex
              ? a_data.scalex
              : a_data.scale
              ? a_data.scale
              : null;
            a_data.scaley = a_data.scaley
              ? a_data.scaley
              : a_data.scale
              ? a_data.scale
              : null;
            a_data.scalez = a_data.scalez
              ? a_data.scalez
              : a_data.scale
              ? a_data.scale
              : null;
            _this.set_scale(
              model.id,
              model.scalex +
                (a_data.scalex !== undefined ? a_data.scalex * inc_const : 0),
              model.scaley +
                (a_data.scaley !== undefined ? a_data.scaley * inc_const : 0),
              model.scalez +
                (a_data.scalez !== undefined ? a_data.scalez * inc_const : 0)
            );
          }
          break;
      }
      ...
      switch (key) {
        ...
        case "z":
          if (!done_position) {
            ...
            _this.set_position(
              model.id,
              model.x +
                (a_data.x !== undefined ? a_data.xtotal * p_from_last_time : 0),
              model.y +
                (a_data.y !== undefined ? a_data.ytotal * p_from_last_time : 0),
              model.z +
                (a_data.z !== undefined ? a_data.ztotal * p_from_last_time : 0)
            );
          }
          break;
        ...
        case "rotationz":
          if (!done_rotation) {
            ...
            if (a_data.rotxtotal === undefined)
              a_data.rotxtotal = a_data.rotationx - rot.x;
            if (a_data.rotytotal === undefined)
              a_data.rotytotal = a_data.rotationy - rot.y;
            if (a_data.rotztotal === undefined)
              a_data.rotztotal = a_data.rotationz - rot.z;
            _this.rotate(
              model.id,
              a_data.rotationx !== undefined
                ? a_data.rotxtotal * p_from_last_time
                : 0,
              a_data.rotationy !== undefined
                ? a_data.rotytotal * p_from_last_time
                : 0,
              a_data.rotationz !== undefined
                ? a_data.rotztotal * p_from_last_time
                : 0
            );
          }
          break;
        ...
        case "scalez":
          if (!done_scale) {
            ...
            a_data.scalez = a_data.scalez
              ...
              ? a_data.scale
              : null;
            if (a_data.scalextotal === undefined)
              a_data.scalextotal = a_data.scalex - model.scalex;
            if (a_data.scaleytotal === undefined)
              a_data.scaleytotal = a_data.scaley - model.scaley;
            if (a_data.scaleztotal === undefined)
              a_data.scaleztotal = a_data.scalez - model.scalez;
            _this.set_scale(
              model.id,
              model.scalex +
                (a_data.scalex !== undefined
                  ? a_data.scalextotal * p_from_last_time
                  : 0),
              model.scaley +
                (a_data.scaley !== undefined
                  ? a_data.scaleytotal * p_from_last_time
                  : 0),
              model.scalez +
                (a_data.scalez !== undefined
                  ? a_data.scaleztotal * p_from_last_time
                  : 0)
            );
          }
          break;
  ...
  this.animate_model = function (model_id, animation) {
    if (_this.models_ref[model_id] === undefined)
      return _this.model_error("animate-model - id not found: " + model_id);
  ...
  this.init = function () {
    ...
    if (!_this.init_done) {
      ...
      switch (_this.controls_type) {
        ...
        default: //OrbitControls
          _this.controls = new THREE.OrbitControls(
            _this.camera,
            _this.renderer.domElement
          );
    ...
    if (_this.mouse_zoom === false) _this.set_mouse_zoom(_this.mouse_zoom);

    if (_this.camera_state) _this.set_camera_state(_this.camera_state);
    else _this.camera.position.set(_this.camerax, _this.cameray, _this.cameraz);
  ...
  this.set_auto_resize = function (b) {
    ...
    if (b) window.addEventListener("resize", _this.do_resize);
  };
  ...
  this.vf_to_geo = function (vertices, faces, colors, default_face_color) {
    ...
    if (!colors) {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (typeof faces[i][3] === "undefined") {
          if (!default_face_color)
            default_face_color = _this.default_face_color; //if user didn't define model color, it will take the default value. since model is colored (if we're here) - we have to give each face a color, or it will appear black
          face.color.setRGB(
            parseInt(_this.default_face_color.substr(1, 2), 16) / 255,
            parseInt(_this.default_face_color.substr(3, 2), 16) / 255,
            parseInt(_this.default_face_color.substr(5, 2), 16) / 255
          );
        } else face.color.setRGB(faces[i][3], faces[i][4], faces[i][5]);
        geo_faces.push(face);
      }
    }
    ...
    if (_this.center_models) geo.center(geo);
  ...
  this.handleFileDrop = function (e) {
    ...
    if (e.dataTransfer.files.length > 0) {
      ...
      if (_this.on_model_drop)
        _this.on_model_drop(e.dataTransfer.getData("Text"));
    }
  };
  ...
  this.external_files_loaded = function () {
    _this.ready = true;
    _this.init();
    if (_this.ready_callback) _this.ready_callback();
  };
  ...
  this.load_three = function (path) {
    ...
    var scripts_to_load = [
      path + "three.min.js",
      path + "webgl_detector.js",
      path + "Projector.js",
      path + "CanvasRenderer.js",
      path +
        (_this.controls_type == 0
          ? "OrbitControls.js"
    ...
    _this.scripts_loader.load_scripts(
      scripts_to_load,
      _this.external_files_loaded
    );
  };
  ...
  this.dispose = function () {
    ...
    if (_this.renderer)
      //one last render (to clean it all)
  ...
  if (_this.ready) {
    ...
    } else _this.model_error("No THREE files were loaded");
  }
}
